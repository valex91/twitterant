...
const previousExecution = now.minus({ hour: 7, minute: 59 }).toISO()

export type TweetBatch = { actionsInBatch: number, batch: RelevantTweetInfo[], executionTime?: string }
...
export const seeker = async () => {
    ...
    try {
        ...
        return relevantTweetDataBatches.map((batch, index) => {
            if (index === 0) {
                return {
                    ...batch,
                    executionTime: now.plus({ minutes: randomInInterval(2, 5) }).toISO({ includeOffset: false })
                }
            } else {
                return {
                    ...batch,
                    executionTime: now.plus({ minutes: (randomInInterval(15, 25) * (index + 1)) }).toISO({ includeOffset: false })
                }
            }
